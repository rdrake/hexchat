...
| 10 | **Per-window unread counts** ✅ | Each `WindowSession` tracks its own `[UUID: Int] unread` (volatile, no persistence). `EngineController.recordActivity(on:)` bumps it for every registered window not currently focused on the activity's session. `WindowSession.focusedSessionID didSet` clears the entry for the new focus before delegating to `recordFocusTransition`. The sidebar reads `controller.unreadBadge(forSession:window:) -> max(perWindow, global)` so cold-launch fallback continues to surface the persisted global counter. New controller-side weak window registry (`weakWindows`) symmetric to `focusRefcount`; `WindowSession.init` registers, `deinit` unregisters synchronously. Session REMOVE scrubs stale UUID keys from every window's `unread` map. | Low | [docs/plans/2026-04-26-per-window-unread.md](2026-04-26-per-window-unread.md) |
| 11 | **Dock-tile unread badge** ✅ | `EngineController.dockBadgeText` sums all `ConversationState.unread` (nil/numeric/"99+"). New `DockBadgeBinder` uses `withObservationTracking` to write `NSApp.dockTile.badgeLabel` on every change. Holds controller weakly; created once via `@State` in `HexChatAppleShellApp.init`. | Low | [docs/plans/2026-04-27-phase-11-dock-badge.md](2026-04-27-phase-11-dock-badge.md) |
| 12 | **IRCv3 `draft/read-marker` cross-device sync** ✅ | C-side cap was already negotiated, MARKREAD parsed, and on-join query sent — Phase 12 is bridge-layer only. New `HC_APPLE_EVENT_READ_MARKER = 8` event variant carries `(connection_have_readmarker, read_marker_timestamp_ms)` from `fe_set_marker_from_timestamp` to Swift. Swift `handleReadMarkerEvent` idempotently advances `ConversationState.lastReadAt` and zeroes `unread` only when the inbound timestamp is strictly newer than the stored value (older replays and zero timestamps are no-ops). New `ReadMarkerBridge` protocol + `CRuntimeReadMarkerBridge` wrap `hc_apple_runtime_send_markread`, which dispatches via `g_main_context_invoke` and re-checks `serv->have_read_marker && serv->connected` on the engine thread (cap-loss race protection). `markReadInternal` extension sends outbound MARKREAD when `Connection.haveReadMarker` is true. Phase 11 dock badge and Phase 10 per-window unread become cross-device-accurate without further changes. | Low | [docs/plans/2026-04-27-phase-12-read-marker.md](2026-04-27-phase-12-read-marker.md) |
...
- **Phase 7.5 — IRCv3 `draft/chathistory` server bridge** ✅ shipped. `loadOlder` now returns `LoadOlderResult { localCount, requestedRemote }` and dispatches `CHATHISTORY BEFORE` via `hc_apple_runtime_request_chathistory_before` when local rows fall short of the requested limit and the connection advertises the cap. Replays land via the existing event channel; SQLite's partial UNIQUE INDEX on `(network_id, channel_lower, server_msgid, timestamp_ms)` dedups across reconnects. See [phase 7.5 plan](2026-04-25-data-model-phase-7-5-chathistory-bridge.md).
- **Phase 8 — Transferable + multi-window** ✅ shipped. `ChatSession`, `ChatUser`, `Network`, `Connection`, `ChatMessage` conform to `Transferable` via JSON `CodableRepresentation` plus a plain-text `ProxyRepresentation`. Per-window `WindowSession` (`@Observable`, weak controller) holds `focusedSessionID`; the first-opened window's `isPrimary` flag mirrors focus to `controller.selectedSessionID` synchronously so the legacy single-window code path stays equivalent. `EngineController` gained `for sessionID:` overloads on `visibleMessages`/`visibleUsers`/`draftBinding`/`prefillPrivateMessage`/`send`. Two concrete drop integrations land: sidebar refocus on dropped `ChatSession` (typed-rejecting unknown UUIDs), and user-pane → input drop prefilling `/msg <nick>`. `Cmd+Opt+T` opens a fresh window seeded via `openWindow(id: "main", value: id)` driven by `@FocusedValue(\.focusedSessionID)`. `@SceneStorage("focusedSessionID")` persists per-window focus across SwiftUI state restoration. See [phase 8 plan](2026-04-26-data-model-phase-8-transferable-multi-window.md).
